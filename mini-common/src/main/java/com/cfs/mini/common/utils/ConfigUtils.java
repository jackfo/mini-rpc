        ...
        if (PROPERTIES == null) {
            synchronized (ConfigUtils.class) {
                if (PROPERTIES == null) {
                    ...
                    String path = System.getProperty(Constants.MINI_PROPERTIES_KEY);
                    if (path == null || path.length() == 0) {
                        // 【找不到】其次，从系统环境变量，获得配置文件的地址
                        path = System.getenv(Constants.MINI_PROPERTIES_KEY);
                        if (path == null || path.length() == 0) {
                            // 【找不到】默认，使用 classpath 下的 `mini.properties` 配置文件
                            path = Constants.DEFAULT_MINI_PROPERTIES;
                        }
                    }
